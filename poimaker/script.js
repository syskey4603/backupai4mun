...
async function test() {
  ...
  if(cb.checked == true) {
    if(freezedate.value != "") {
      prompt = "give me 50 different instances of different countries governments violating the " + legality.value + " make it simple but include the real instance with names and dates but make it for all 50 of the main countries dont give general violations make it extremely specific with real events that took place also add a line of space between each violation with a colon to seperate the country ansd the violation remember to be very specific and give the real names and dates also do not research for information that was found after " + freezedate.value + "all information has to be from before " + freezedate.value + " do not in any circumstance make up any of claims and give real accesible sources if u cannot find the link just provide a search term so we can find the information on an external website all of the information has to be from websites that currently exist";
    }
    else {
      console.log("no freezedate")
    prompt = "give me 50 different instances of different countries governments violating the " + legality.value + " make it simple but include the real instances with real names and dates but make it for all 50 of the main countries dont give general violations make it extremely specific with real events that took place also add a line of space between each violation with a colon to seperate the country ansd the violation remember to be very specific and give the real names and dates add one line of space between each country do not in any circumstance make up any of claims and give real accesible sources if u cannot find the link just provide a search term so we can find the information on an external website all of the information has to be from websites that currently exist";
  }
}
  else {
    if(freezedate.value != "") {
      prompt = "5 specific instances with details of the actual example including the peoples names and other details of the violations by " + country.value + " in regards to the legality of " + legality.value + " do not find anything after this date " + freezedate.value + " no information can be taken from after that date make it very detailed add the actual names of people and expand on the real life example cite your sources too with the link make sure the link actually works and does not return a not found error also add the specific article being violated with the date of the violations and add one line of line gap between each scentence do not in any circumstance make up any of claims and give real accesible sources if u cannot find the link just provide a search term so we can find the information on an external website all of the information has to be from websites that currently exist";
    }
    else {
  prompt = "5 specific instances with details of the actual example including the peoples names and other details of the violations by " + country.value + " in regards to the legality of " + legality.value + " the violations have to be by the countries government breaking international law make it very detailed add the actual names of people and expand on the real life example cite your sources too with the link also add the specific article being violated with the date of the violations and add one line of line gap between each scentence do not in any circumstance make up any of claims and give real accesible sources if u cannot find the link just provide a search term so we can find the information on an external website all of the information has to be from websites that currently exist";
  }
}
